(() => {
  ...
  style.textContent = `
    ...
    .lt-news-form{display:grid;grid-template-columns:150px 130px minmax(220px,1fr) auto;gap:9px;margin-top:14px}
    .lt-news-form label{display:flex;flex-direction:column;gap:5px;font-size:8px;color:var(--muted)}
    .lt-news-form input{width:100%;box-sizing:border-box;border:1px solid var(--line);background:#06100b;color:var(--text);border-radius:9px;padding:10px;font:inherit;outline:none}
    .lt-news-form input:focus{border-color:#528b67}.lt-news-add{align-self:end;border:1px solid #6f472e;background:#20130b;color:#f2bd8d;border-radius:9px;padding:10px 13px;font-weight:800;cursor:pointer;white-space:nowrap}
    ...
    @media(max-width:760px){.lt-news-form{grid-template-columns:1fr 1fr}.lt-news-form label:nth-child(3),.lt-news-add{grid-column:1/-1}.lt-news-row{grid-template-columns:1fr auto}.lt-news-time,.lt-news-name{grid-column:1}.lt-news-class,.lt-news-remove{grid-column:2}.lt-news-next{align-items:flex-start}.lt-news-countdown b{font-size:14px}}
  `;
  ...
  card.innerHTML = `
    <div class="lt-news-head">
      <div><p class="eyebrow">WEEKLY RED-FOLDER NEWS</p><h3>High-impact USD protection</h3><p>Every Sunday, copy only the USD RED events from Forex Factory. Enter the date, exact UK time and event name as shown. EVE converts BST/GMT internally and manages the blackout automatically.</p></div>
    ...
    <form class="lt-news-form" id="ltNewsForm">
      ...
      <button class="lt-news-add" id="ltNewsAdd" type="submit">Add RED USD event</button>
    </form>
    <p class="lt-news-guidance">Standard RED: 30 min before → 15 min after. Major CPI/PCE/NFP/FOMC/rate/Powell events: 45 min before → 30 min after. Pending ideas are suspended; triggered trades keep their exact locked stop and target.</p>
  ...
  function eventRow(event) {
    ...
    const policy = major ? '45 before / 30 after' : '30 before / 15 after';
    return `<div class="lt-news-row">
      <div class="lt-news-time"><span>${safe(stamp.date)}</span><b>${safe(stamp.time)} UK</b></div>
      <div class="lt-news-name"><strong>${safe(event.event_name)}</strong><small>USD RED · blackout ${safe(blackoutText(event))} · ${safe(policy)}</small></div>
      <span class="lt-news-class ${major ? 'major' : ''}">${major ? 'MAJOR' : 'HIGH'}</span>
  ...
  function maybeSpeak(news, state) {
    ...
    if (status === 'blackout' && previousStatus !== 'blackout' && window.eveLiveVoice?.say) {
      const events = (news.active_events || []).map(item => item.event_name).filter(Boolean).join(' and ') || 'high-impact USD news';
      ...
      const text = activeTrade
        ? `Micky, red-folder USD news protection is active for ${events}. Your triggered campaign stays locked to its published stop and target. Elevated news risk remains until ${end} UK time.`
        : `Micky, red-folder USD news protection is active for ${events}. No new gold trade until ${end} UK time.`;
  ...
  function render(news, state = latestState) {
    ...
    pill.textContent = status === 'blackout' ? 'NEWS BLACKOUT' : status === 'armed' ? 'ARMED' : status === 'clear' ? 'CLEAR' : 'CALENDAR UNAVAILABLE';
    ...
    if (status === 'unavailable') {
      ...
      nextBox.innerHTML = `<div class="lt-news-next blackout"><div><span>HIGH-IMPACT USD NEWS ACTIVE</span><strong>${safe(active.event_name || 'Red-folder event')}</strong><small>${safe(stamp.time)} UK release · blackout ends ${safe(end.time)} UK. ${safe(handling)}</small></div><div class="lt-news-countdown"><b>BLACKOUT</b><small>news protection</small></div></div>`;
    } else if (latestNews.next_event) {
      const event = latestNews.next_event;
      const stamp = ukParts(event.scheduled_at);
      nextBox.innerHTML = `<div class="lt-news-next"><div><span>NEXT RED USD EVENT</span><strong>${safe(event.event_name)}</strong><small>${safe(stamp.date)} · ${safe(stamp.time)} UK · blackout ${safe(blackoutText(event))}</small></div><div class="lt-news-countdown"><b>${safe(countdownText(latestNews.minutes_to_next_event))}</b><small>until release</small></div></div>`;
    } else {
      nextBox.innerHTML = `<div class="lt-news-next"><div><span>NEWS CALENDAR</span><strong>No upcoming USD RED events loaded</strong><small>Use Forex Factory on Sunday and add this week's red-folder events above.</small></div><div class="lt-news-countdown"><b>CLEAR</b><small>no blackout loaded</small></div></div>`;
    }

    const events = (latestNews.events || []).filter(item => item?.event_id);
    byId('ltNewsList').innerHTML = events.length ? events.map(eventRow).join('') : '<div class="lt-news-empty">No upcoming USD RED events are loaded. Only copy the red USD rows from Forex Factory.</div>';
  ...
  async function submitEvent(event) {
    ...
    try {
      const result = await api('/live-trader/chat', {
        ...
        body:JSON.stringify({message:`__EVE_NEWS_ADD__|${date}|${time}|${name}`}),
      });
  ...
  // Reuse the Historical Academy's existing 5-second Live Trader refresh rather than
  // adding another Netlify request loop. This keeps the news panel current without
  // increasing proxy usage.
