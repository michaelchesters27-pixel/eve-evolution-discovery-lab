(() => {
  ...
  function applySessionCopy() {
    ...
    if (levelMatch) {
      flip.textContent = `${label} session outlook invalidates if price ${levelMatch[2].toLowerCase()} ${levelMatch[3]} and M15/M5 structure remains ${levelMatch[4].toLowerCase()}.`;
      return;
    }
    ...
    if (structureMatch) {
      flip.textContent = `${label} session outlook invalidates if M15/M5 structure and momentum both turn ${structureMatch[2].toLowerCase()}.`;
    }
  }
