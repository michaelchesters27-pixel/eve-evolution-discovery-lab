...
def path_metrics(campaign: dict[str, Any], bars: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    out: dict[str, Any] = {"path_bars": len(bars), "mfe_r": None, "mae_r": None, "post_completion_best_r": None, "target_reached_after_completion": False, "time_to_trigger_minutes": None, "active_minutes": None, "risk_price": round(risk, 5) if risk else None}
    ...
    if side not in {"BUY", "SELL"} or not entry or not risk or not completed:
        return out
    if triggered:
        terminal_minute = completed.replace(second=0, microsecond=0)
        mfe = mae = 0.0
        for bar in bars:
            stamp = bar_time(bar)
            if not stamp or stamp + timedelta(minutes=1) <= triggered or stamp >= terminal_minute:
                continue
...
def diagnose(campaign: dict[str, Any], metrics: dict[str, Any], opinion: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if status in {"invalidated", "expired"} and not campaign.get("triggered_at"): entry_timing = "never_triggered"
    elif status == "lost" and wait is not None and float(wait) >= 90 and active is not None and float(active) <= 20: entry_timing = "aged_setup_triggered_late"
    elif status == "lost" and active is not None and float(active) <= 10 and mfe < .2: entry_timing = "rapid_failure_after_entry"
    elif mfe >= .5: entry_timing = "entry_worked_initially"
    else: entry_timing = "mixed_or_unproven"
    if status == "lost" and (target_after or post >= 2): stop_diag = "timing_or_stop_geometry_suspect"
    elif status == "lost" and mfe >= 1: stop_diag = "gave_back_one_r_or_more_before_stop"
    elif status == "lost" and active is not None and float(active) <= 10 and mfe < .2: stop_diag = "rapid_stop_failure"
    elif status == "lost": stop_diag = "not_isolated_from_entry_or_direction"
    else: stop_diag = "not_applicable"
    if status == "won": target_diag = "reached"
    elif target_after: target_diag = "reachable_after_stop"
    elif status == "lost" and rr >= 4 and max(mfe, post) < 1: target_diag = "aggressive_and_unproven"
    elif status == "lost" and rr > 0 and mfe >= rr*.75: target_diag = "nearly_reached_before_failure"
    elif status == "lost": target_diag = "not_reached"
    else: target_diag = "not_applicable"
    if status == "won": primary, confidence = "successful_execution", "high"
    elif status in {"invalidated", "expired"} and not campaign.get("triggered_at"): primary, confidence = "setup_selection_or_entry_timing", "high"
    elif status == "lost" and (target_after or post >= 2): primary, confidence = "execution_timing_or_stop", "high"
    elif status == "lost" and entry_timing == "aged_setup_triggered_late": primary, confidence = "aged_setup_trigger", "high"
    elif status == "lost" and direction is False and mfe < .2: primary, confidence = "direction_and_execution_failed", "medium"
    elif status == "lost" and mfe >= 1: primary, confidence = "risk_management_or_stop", "medium"
    elif status == "lost": primary, confidence = "normal_or_unresolved_loss", "low"
    else: primary, confidence = "untriggered_setup_evidence", "medium"
    lessons = {
