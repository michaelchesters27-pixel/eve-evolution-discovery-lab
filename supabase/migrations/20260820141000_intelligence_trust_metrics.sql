-- Make EVE Intelligence distinguish built, fully-labelled and integrity-audited M5 rows.
-- The Scientist researches complete_only=true rows, while the hard audit maintains
-- its own incremental audited-row counter. Those are different populations and
-- must not be displayed as if they were the same number.
